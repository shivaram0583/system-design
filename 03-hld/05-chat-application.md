...
```mermaid
graph TD
    Client[Client App] <-->|WebSocket| WS[WS Gateway<br/>10K servers, Connection Mgr]
    WS --> Chat[Chat Service<br/>message routing + ordering]
    Chat --> Kafka[Kafka<br/>message fanout]
    Chat --> Cass[(Cassandra<br/>messages, chat meta)]
    Chat --> Redis[(Redis<br/>presence, sessions, routing)]
    Kafka --> Push[Push Notification Service]
    Chat --> S3[S3 / CDN<br/>media]
```
